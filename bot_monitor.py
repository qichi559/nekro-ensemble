...
function botName(i){var b=BOTS_DATA[i-1];return b?b.role:("Bot "+i)}
...
function appendMsg(msg){
  ...
  var avUrl=msg.role==="user"?"https://q1.qlogo.cn/g?b=qq&nk="+USER_QQ+"&s=100&t=20260816":"https://q1.qlogo.cn/g?b=qq&nk="+botQ(currentBot)+"&s=100&t=20260816";
  var avFb=msg.role==="user"?"🙂":"🦋";
  var html="<div class='msg-avatar'><img class='avatar-img' src='"+avUrl+"'><span class='avatar-fallback'>"+avFb+"</span></div>";
...
function switchBot(i){
  ...
  if(img)img.src="https://q1.qlogo.cn/g?b=qq&nk="+botQ(i)+"&s=640&t=20260816";
...
.card-header-left>div{overflow:hidden;min-width:0;display:flex;align-items:center;gap:6px;flex-wrap:nowrap}
.card-header-left>div .status-badge{padding:2px 8px;font-size:0.62rem;flex-shrink:0;line-height:1.5}
.bot-name{font-size:1.05rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100%;flex-shrink:1}
.card-header-right{display:flex;align-items:center;flex-shrink:0}
.main-btn-chat{padding:9px 16px;font-size:.85rem}
.bot-avatar{width:42px;height:42px}
...
/* ===== 模型卡片美化 ===== */
.model-card{background:var(--pink-card);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);border:1px solid var(--pink-border);border-radius:14px;padding:14px 16px;position:relative;overflow:hidden}
...
.model-head{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:12px}
.model-head .label{font-size:.78rem;font-weight:600;color:#be185d;letter-spacing:1px;text-transform:uppercase;flex-shrink:0}
.model-badge{display:inline-flex;align-items:center;gap:5px;background:linear-gradient(135deg,rgba(236,72,153,.14),rgba(217,70,239,.14));border:1px solid rgba(236,72,153,.28);color:#be185d;font-size:.72rem;font-weight:700;padding:4px 12px;border-radius:20px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100%}
.model-badge::before{content:"";width:7px;height:7px;border-radius:50%;background:#ec4899;flex-shrink:0;box-shadow:0 0 6px rgba(236,72,153,.8);animation:modelPulse 1.6s ease infinite}
@keyframes modelPulse{0%,100%{opacity:1}50%{opacity:.35}}
.model-row{background:rgba(255,255,255,.5);backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px);border-radius:10px;padding:10px 12px;margin-bottom:10px}
.model-row .model-row-label{font-size:.68rem;color:#be185d;font-weight:600;margin-bottom:6px;letter-spacing:.5px}
.model-row .model-row-label .hint{color:#f0a6c9;font-weight:400}
...
.model-btns{display:flex;gap:8px}
...
function loadModelGroups(){fetch("/api/model-presets").then(function(r){return r.json()}).then(function(d){modelGroups=d.groups||[];currentModel=d.current||"unknown";_renderModelCard()}).catch(function(){})}function _renderModelCard(){var el=document.getElementById("model-section");if(!el)return;var cur=null;(modelGroups||[]).forEach(function(g){if(g.group_name===currentModel)cur=g});var curPretty=cur?cur.pretty:currentModel;setSummary("summary-model",curPretty);if(!modelGroups.length){el.innerHTML='<div class="model-card"><div class="model-head"><span class="label">当前模型组</span><span class="model-badge">未配置</span></div><div class="model-row"><div class="model-row-label">暂无可用模型组，请先添加</div></div><div class="model-btns"><button class="model-btn model-btn-add" onclick="showAddModelGroup()">＋ 添加模型组</button></div></div>';return}var opts=(modelGroups||[]).map(function(g){var sel=g.group_name===currentModel?" selected":"";return '<option value="'+esc(g.group_name)+'"'+sel+'>'+esc(g.pretty||g.group_name)+'</option>'}).join("");var curChip=cur?('<div class="model-meta-chip">模型 <b>'+esc(cur.chat_model||"-")+'</b></div><div class="model-meta-chip" title="'+esc(cur.base_url||"")+'">接口 '+(cur.base_url?esc(cur.base_url.replace(/^https?:\/\//,"").replace(/\/+$/,"")):"-")+'</div>'):'';el.innerHTML='<div class="model-card"><div class="model-head"><span class="label">当前模型组</span><span class="model-badge">'+esc(curPretty)+'</span></div>'+(cur?(''):'')+'<div class="model-row"><div class="model-row-label">切换模型组 <span class="hint">（所有 Bot 同步切换）</span></div><select class="model-select" id="model-select" onchange="switchModel(this.value)">'+opts+'</select></div>'+(curChip?'<div class="model-meta">'+curChip+'</div>':'')+'<div class="model-btns" style="margin-top:10px"><button class="model-btn model-btn-switch" id="model-apply-btn" onclick="switchModel(document.getElementById(\'model-select\').value)">⚡ 应用所选</button><button class="model-btn model-btn-add" onclick="showAddModelGroup()">＋ 添加组</button><button class="model-btn model-btn-del" onclick="deleteCurrentModelGroup()">－ 删除组</button></div></div>'}
...
class MonitorHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path.startswith("/api/chat/tts-config"):
            ...
            self.end_headers()
            bots_js = json.dumps([{"qq": b.get("qq", ""), "role": b.get("role", "")} for b in BOTS], ensure_ascii=False)
