...
function loadModelGroups(){fetch("/api/model-presets").then(function(r){return r.json()}).then(function(d){modelGroups=d.groups||[];currentModel=d.current||"unknown";var el=document.getElementById("model-section");if(!el)return;var cur=null;(modelGroups||[]).forEach(function(g){if(g.group_name===currentModel)cur=g});var curPretty=cur?cur.pretty:currentModel;setSummary("summary-model",curPretty);var opts=(modelGroups||[]).map(function(g){var sel=g.group_name===currentModel?" selected":"";return '<option value="'+esc(g.group_name)+'"'+sel+'>'+esc(g.pretty)+'</option>'}).join("");el.innerHTML='<div class="info-row"><span class="label">当前模型组</span><select onchange="switchModel(this.value)" style="flex:1;background:#ffffff;border:1px solid rgba(180,140,200,.45);border-radius:8px;padding:9px 12px;color:#000000;font-size:0.9rem;cursor:pointer;outline:none">'+opts+'</select>'+(modelGroups.length?'<button class="sub-btn" onclick="deleteCurrentModelGroup()" style="margin-left:8px;background:#7f1d1d;color:#fbeaea;border:none;border-radius:6px;padding:7px 12px;cursor:pointer;font-size:.8rem" title="删除当前选中的模型组（所有 Bot 同步删除）">删除组</button>':'')+'<button class="sub-btn" onclick="showAddModelGroup()" style="margin-left:8px;background:#1d4e7f;color:#eaf3fb;border:none;border-radius:6px;padding:7px 12px;cursor:pointer;font-size:.8rem" title="在所有 Bot 上添加新模型组">添加组</button></div>';}).catch(function(){})}var _addCtx={};function showAddModelGroup(){var n=prompt("新模型组名称（所有 Bot 同步创建）：");if(!n||!n.trim())return;_addCtx.name=n.trim();var b=prompt("BASE_URL 接口地址（如 https://api.deepseek.com/v1）：");if(!b||!b.trim())return;_addCtx.base_url=b.trim();var k=prompt("API_KEY（可留空，拉取模型列表需要）：","");_addCtx.api_key=(k||"").trim();showToast("正在拉取模型列表...","");fetch("/api/fetch-models",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({base_url:_addCtx.base_url,api_key:_addCtx.api_key})}).then(function(r){return r.json()}).then(function(d){if(!d.ok){showToast("拉取失败: "+(d.msg||""),"error");return}var models=d.models||[];if(models.length===0){showToast("接口返回模型列表为空","error");return}var sel=document.getElementById("model-section");if(!sel)return;var opts=models.map(function(m){return '<option value="'+esc(m)+'">'+esc(m)+'</option>'}).join("");sel.innerHTML='<div class="info-row"><span class="label">选择模型</span><select id="model-picker" style="flex:1;background:#ffffff;border:1px solid rgba(180,140,200,.45);border-radius:8px;padding:9px 12px;color:#000000;font-size:0.9rem;cursor:pointer;outline:none">'+opts+'</select><button class="sub-btn" onclick="confirmAddModel()" style="margin-left:8px;background:#1d4e7f;color:#eaf3fb;border:none;border-radius:6px;padding:7px 12px;cursor:pointer;font-size:.8rem">确认添加</button><button class="sub-btn" onclick="cancelAddModel()" style="margin-left:8px;background:#666;color:#fff;border:none;border-radius:6px;padding:7px 12px;cursor:pointer;font-size:.8rem">取消</button></div>';}).catch(function(e){showToast("请求失败:"+e.message,"error")})}function confirmAddModel(){var picker=document.getElementById("model-picker");if(!picker)return;var m=picker.value;if(!m)return;showToast("正在创建 "+_addCtx.name+"...","");fetch("/api/create-model-group",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({group_name:_addCtx.name,chat_model:m,base_url:_addCtx.base_url,api_key:_addCtx.api_key})}).then(function(r){return r.json()}).then(function(d){showToast(d.msg,d.ok?"success":"error");loadModelGroups()}).catch(function(e){showToast("请求失败:"+e.message,"error")})}function cancelAddModel(){loadModelGroups()}
...
function applyModel(i){var p=modelGroups[i];if(!p)return;currentModel=p.group_name;renderModel({groups:modelGroups,current:currentModel});showToast("正在切换...","");fetch("/api/apply-model",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({group_name:p.group_name,chat_model:p.chat_model,base_url:p.base_url,api_key:p.api_key})}).then(function(r){return r.json()}).then(function(d){showToast(d.msg,d.ok?"success":"error");setTimeout(loadModel,3000)}).catch(function(e){showToast("请求失败:"+e.message,"error")})}
