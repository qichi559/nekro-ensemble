...
.model-head{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:10px;flex-wrap:wrap}
...
.model-meta{display:flex;gap:6px;flex-wrap:wrap;justify-content:flex-end;min-width:0}
.model-meta-chip{background:rgba(236,72,153,.07);border:1px dashed rgba(236,72,153,.25);color:#be185d;font-size:.68rem;padding:3px 10px;border-radius:14px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100%}
...
function loadModelGroups(){fetch("/api/model-presets").then(function(r){return r.json()}).then(function(d){modelGroups=d.groups||[];currentModel=d.current||"unknown";selectedModel=currentModel;_renderModelCard()}).catch(function(){})}function _renderModelCard(){var el=document.getElementById("model-section");if(!el)return;if(!modelGroups||!modelGroups.length){setSummary("summary-model","未配置");el.innerHTML='<div class="model-card"><div class="model-head"><span class="model-badge">未配置</span></div><div class="model-tip">暂无可用模型组，请先添加</div><div class="model-btns"><button class="model-btn model-btn-add" onclick="showAddModelGroup()">＋ 添加模型组</button></div></div>';return}var cur=null;(modelGroups||[]).forEach(function(g){if(g.group_name===currentModel)cur=g});var curPretty=cur?cur.pretty:currentModel;setSummary("summary-model",curPretty);var opts=(modelGroups||[]).map(function(g){var sel=(g.group_name===(selectedModel||currentModel))?" selected":"";return '<option value="'+esc(g.group_name)+'"'+sel+'>'+esc(g.pretty||g.group_name)+'</option>'}).join("");var pending=selectedModel&&selectedModel!==currentModel;var curChip=cur?('<div class="model-meta-chip" title="'+esc(cur.base_url||"")+'">'+esc(cur.chat_model||"-")+' · '+(cur.base_url?esc(cur.base_url.replace(/^https?:\/\//,"").replace(/\/+$/,"")):"-")+'</div>'):'';el.innerHTML='<div class="model-card"><div class="model-head"><span class="model-badge" title="当前使用的模型组">'+esc(curPretty)+'</span>'+(curChip?'<div class="model-meta">'+curChip+'</div>':'')+'</div><select class="model-select" id="model-select" onchange="selectModel(this.value)">'+opts+'</select><div class="model-btns"><button class="model-btn model-btn-switch" id="model-apply-btn" onclick="applySelectedModel()"'+(pending?"":" disabled")+'>'+(pending?'⚡ 应用所选':'已是最新')+'</button><button class="model-btn model-btn-add" onclick="showAddModelGroup()">＋ 添加</button><button class="model-btn model-btn-del" onclick="deleteCurrentModelGroup()">－ 删除</button></div></div>'}
