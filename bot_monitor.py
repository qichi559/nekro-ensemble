...
function deleteCurrentModelGroup(){var name=selectedModel||currentModel;if(!name||name==="unknown"){showToast("请先在下拉框选中要删除的模型组","error");return}if(!confirm("确认删除模型组「"+name+"」？"))return;showToast("正在删除 "+name+" ...","");fetch("/api/delete-model-group",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({group_name:name})}).then(function(r){return r.json()}).then(function(d){showToast(d.msg,d.ok?"success":"error");if(d.ok){if(name===currentModel)currentModel="unknown";selectedModel="";loadModelGroups()}}).catch(function(e){showToast("请求失败:"+e.message,"error")})}
...
function renderNavLinks(navLinks){var links=[{name:"🎨 Nai2API 工作台",url:"https://nai.sta1n.cn/"}].concat(navLinks||[]);setSummary("summary-nav",links.length+"个链接");var html='<div class="nav-grid">';links.forEach(function(nav){var colorClass=nav.name.indexOf("酒馆")>=0?"nav-btn-tavern":(nav.name.indexOf("面板")>=0||nav.name.indexOf("Nai2API")>=0)?"nav-btn-panel":"main-btn-napcat";html+='<a href="'+esc(nav.url)+'" target="_blank" class="nav-btn '+colorClass+'">'+esc(nav.name)+'</a>'});html+='</div>';document.getElementById("extra-nav-section").innerHTML=html}
