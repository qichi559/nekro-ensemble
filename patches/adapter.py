...
class OnebotV11Adapter(BaseAdapter[OnebotV11Config]):
    ...
    async def _send_message(self, request: PlatformSendRequest) -> Optional[str]:
        """发送普通消息（文本、@、图片等）"""
        import re as _re
        _EMO_TAG = _re.compile(r"\[\[emotion:([^\]\[]*)\]\]")
        _EMO_DANGLE = _re.compile(r"\[\[emotion:[^\n\]\[]*")

        def _strip_emotion(text):
            if not text:
                return text or "", None
            m = _EMO_TAG.search(text)
            raw = m.group(1) if m else ""
            m2 = _re.match(r"[a-zA-Z0-9_\-]+", raw)
            emotion = m2.group(0).lower() if m2 else None
            clean = _EMO_TAG.sub("", text)
            clean = _EMO_DANGLE.sub("", clean).strip()
        ...
        voice_text, _voice_emotion = _strip_emotion(_raw_voice)
        ...
        try:
            ...
            _base = _os.environ.get("NEKRO_DATA_DIR") or str(OsEnv.DATA_DIR)
            _sw = _Path(_base) / "voice_switch"
            if _sw.exists():
            ...
            try:
                ...
                def _gen_voice():
                    ...
                    try:
                        req = _urlreq.Request(
                            _tts_url,
                            data=_json.dumps({"text": voice_text, "emotion": _voice_emotion or ""}).encode(),
                            headers={"Content-Type": "application/json"},
                            method="POST",
                        )
                        with _urlreq.urlopen(req, timeout=30) as resp:
                            d = _json.loads(resp.read().decode())
                        if d.get("ok") and d.get("audio"):
                            return _b64.b64decode(d["audio"])
                    except Exception:
                        pass
                    return None

                _audio = await _asyncio.to_thread(_gen_voice)
                if _audio:
                    voice_msg = Message()
                    voice_msg.append(MessageSegment.record(file="base64://" + _b64.b64encode(_audio).decode()))
                    sent_id = await self._send_to_chat(request.chat_key, voice_msg)
            except Exception:
                pass
        return sent_id
